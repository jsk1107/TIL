...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        for i in range(4):
            # cv2.norm: 유클리디안 거리를 뜻한다. x, y는 마우스 클릭할때의 좌표값.
...
""" 모서리 점들의 좌표와 드래그 상태 플래그 """
srcQuad = np.array([[30, 30], [30, h-30], [w-30, h-30], [w-30, 30]], np.float32) # 초기좌표
dstQuad = np.array([[0, 0], [0, dh-1], [dw-1, dh-1], [dw-1, 0]], np.float32) # 옮긴후좌표
