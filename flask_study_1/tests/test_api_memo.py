...
def test_get_memos(client, memo_data):
    r = client.get(
        '/api/memos',
        follow_redirects=True
    )
...
def test_put_memo(client, memo_data):
    new_data = {'title': 'new_title',
                'content': 'new_content'}

    r = client.put('/api/memos/1', data=new_data)
    assert r.status_code == 200
...
def test_delete_memo(client):
    ...
    r = client.get('/api/memos', follow_redirects=True
    )
...
def test_post_memo_with_img(client, memo_data):
    ...
    data['linked_image'] = (BytesIO(b'dummy'), 'test.jpg')
    r = client.post(
        '/api/memos',
        data=data
    )
...
def test_put_memo_with_img(client, memo_data):
    ...
    data = {
        'linked_image': (BytesIO(b'dummy'), 'test.jpg')
    }
